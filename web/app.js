...
async function showSettings(external = false) {
  showDialog(external ? '外部窗口' : '连接与设置', `<p class="dialog-description">日常工作在本客户端完成。原工作台与模拟器默认隐藏，后台持续同步。</p><div class="settings-row"><div>Codex 连接<small>${state.ready ? '已连接 · 使用本机 ChatGPT 登录' : '尚未连接'}</small></div><button type="button" class="secondary" id="reconnect">重新连接</button></div><div class="settings-row"><div>原生工作台<small>打开完整的原工作台窗口</small></div><button type="button" class="secondary" data-window="show-studio">显示</button></div><div class="settings-row"><div>iOS 模拟器<small>打开独立模拟器窗口</small></div><button type="button" class="secondary" data-window="show-simulator">显示</button></div><div class="settings-row"><div>隐藏所有外部窗口</div><button type="button" class="secondary" data-window="hide">隐藏</button></div><p class="inspector-note">这是接入 Codex App Server 的独立工作客户端。App Server 为实验性接口；官方 Codex 应用保持独立。</p>`, '完成', async () => {});
...
async function refreshEditor(force = false) {
  ...
  try {
    ...
    $('edit-mode').classList.toggle('selected', s.editing); $('run-mode').classList.toggle('selected', !s.editing);

...
- 新工程补齐 ios.classPrefix、四项 permissionKeys 和本地化文件。导出包含所有页面、弹窗、资源和动作；目录自动为 App名-htmlios。

遵守用户测试安排，不自行构建或启动模拟器。交付页面/资源/动作清单、已做检查和待验证项；用户授权运行校验时使用内置 validate.py 和验收清单。

...
有现成 UI 原图时，同时完整阅读 `UI原图复刻规则.md`（ui-replica/4），按审计、所有页面模板与校准、独立资源、确定性布局、模板抽离、截图差分和交互验收的流程生产。图片资源按《生成素材规则.md》只生成，整体视觉必须对齐，允许斑点、细纹理和发丝不同；文字、布局与交互不放宽。该规则不改变本协议的技术与修改边界。
...
页面和图层 ID 必须保持稳定，图片与字体完整登记资源目录。工作台在源码更新时使用稳定 ID 合并未保存外观和新页面结构；删除或改变类型的图层，其旧调整仅归档，不强行应用到新图层。业务 HTML 不应通过重复创建页面、替换稳定 ID 或修改工作区缓存来强制刷新图片库。
...
透明容器的 overflow:hidden 仍会裁切子元素。工作台排序不会自动改变父子关系；调整组合位置应选中父容器。裁切边界仅在编辑态显示，不能作为业务 UI 写入页面。需要将独立资源移出父容器时使用显式操作，外层安全区和滚动裁切保持有效。
...
真实文字与按钮标题使用 color 定义前景颜色。工作台提供独立文案色板与 HEX 输入；缩略图对浅色文字使用对比背景，不能将预览棋盘背景写入页面，也不能把文字烘焙进图片来代替可编辑文案。

### 新增层的默认顺序

工作台添加或复制的资源在目标场景中分配明确的 layerOrder；不得将缺失层级视作零而压到页面容器下面。原生绘制、编辑列表和命中顺序需采用一致的回退规则。排序不会自动解除父容器裁切，也不会自动改变父子关系。

...
extension StudioController {
 ...
 func showSyncStatus(_ text:String){syncStateLabel.text=text.contains("失败") ? "同步有问题" : text.contains("冲突") ? "需要处理冲突" : text.hasPrefix("已同步") ? "● 已同步" : "同步中…";syncStateLabel.textColor=UIColor(studioHex:text.contains("失败") || text.contains("冲突") ? "#9E3B2C":"#38664F");if Date()>statusHoldUntil{footer.text="  "+text}}
}

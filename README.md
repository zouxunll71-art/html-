...
编辑工具快捷键：Command+A 移动、Command+Shift+S 缩放、Command+D 旋转。仅选取与编辑模式生效；输入框、输入法组合输入及拖动过程中不切换工具。Command+S 保存布局。
...
Command+W 切换运行/编辑模式，替代关闭窗口快捷键；Command+Q 恢复系统退出应用行为。
